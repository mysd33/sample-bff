...
public class PKCS12BasicReportSigner implements ReportSigner {
    ...
    public Report sign(final Report originalReport, final SignOptions options) {
        ...
        File signedPdfTempFilePath = tempFileCreator.createTempFile(
            ReportsConstants.PDF_TEMP_FILE_PREFIX,
            ReportsConstants.PDF_FILE_EXTENSION);
        try (var fos = new FileOutputStream(signedPdfTempFilePath)) {
            ...
            var key = (PrivateKey) ks.getKey(alias,
                digitalSignatureConfig.getPkcs12().getPassword().toCharArray());
    ...
    private void doSign(final PdfReader originalPdfReader, final FileOutputStream fos,
        final PrivateKey key,
        final Certificate[] chain, final SignOptions options) {
        try (var pdfStamperWrapper = new PdfStamperWrapper(
            ...
            var sig = new DefaultPdfSignature(
                digitalSignatureConfig.getPkcs12().getHashAlgorithm());
            ...
            pdfStamperWrapper.setEncryption(
                originalPdfReader.is128Key() ? PdfWriter.STANDARD_ENCRYPTION_128
                    : PdfWriter.STANDARD_ENCRYPTION_40,
                options.getUserPassword(), options.getOwnerPassword(),
                originalPdfReader.getPermissions());
        } catch (IOException e) {
    ...
    private void createVisbleSignatureImage(final PdfSignatureAppearance sap,
        final SignOptions options) {
        float[] rect = options.getVisibleSignRect();
        // OpenPDFの座標系は左下が原点でY軸が上方向に伸びる模様なので、Y座標を変換する
        sap.setVisibleSignature(
            new Rectangle(rect[0], Y_AXIS_MAX_VALUE - rect[3], rect[2], Y_AXIS_MAX_VALUE - rect[1]),
